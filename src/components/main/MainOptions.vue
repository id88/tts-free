    ...
    <div class="options">
        <el-form :model="formConfig" label-width="120px" label-position="top">
            ...
            <el-form-item label="语言">
                <el-select-v2 class="languageSelect" v-model="formConfig.languageSelect" placeholder="选择语言" filterable
            ...
            <el-form-item label="语音">
                <el-select v-model="formConfig.voiceSelect" placeholder="选择语音" @change="voiceSelectChange">
                    <el-option v-for="item in voiceSelectList" :key="item.ShortName"
                        :label="item.DisplayName + '-' + item.LocalName" :value="item.ShortName">
                        <div style="display: flex; justify-content: space-between">
                            <span style="margin-right: 5px">{{
                                item.DisplayName + "-" + item.LocalName
                            }}</span>
                            <el-button size="small" type="success" circle @click.stop="audition(item.ShortName)"><el-icon>
                                    <CaretRight />
                                </el-icon></el-button>
                        </div>
                    </el-option>
            ...
            <el-form-item label="说话风格">
                <el-select v-model="formConfig.voiceStyleSelect" placeholder="选择说话风格" :disabled="apiEdge">
                    <el-option v-for="item in voiceStyleSelectList" :key="item" :label="getStyleDes(item)?.word || item"
                        :value="item">
            ...
            <el-form-item label="角色扮演">
                <el-select v-model="formConfig.role" placeholder="选择角色" :disabled="apiEdge">
                    <el-option v-for="item in rolePlayList" :key="item" :label="getRoleDes(item)?.word || item"
                        :value="item">
                        <div style="display: flex; justify-content: start">
                            <span style="margin-right: 5px">{{
            ...
            <el-form-item label="语速">
                <el-slider v-model="formConfig.speed" show-input size="small" :show-input-controls="false" :max="3"
                    :step="0.01" />
            </el-form-item>
            <el-form-item label="音调">
                <el-slider v-model="formConfig.pitch" show-input size="small" :show-input-controls="false" :max="2"
                    :step="0.01" />
            </el-form-item>
            <el-form-item class="startBtn">
                <div class="configOption">
                    <el-button color="#626aef" :dark="false" plain size="small" @click="saveConfig">保存配置</el-button>
                    <el-select-v2 class="get-cfg" v-model="currConfigName" placeholder="选择配置" filterable
                        :options="config.configLabel" @change="configChange"></el-select-v2>
                </div>
